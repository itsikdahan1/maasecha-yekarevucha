...
import Image from 'next/image'; // חוזרים לשיטה הנכונה
...
export const Header: FC = () => {
    ...
    const mobileMenuVariants = {
        ...
        visible: { opacity: 1, y: "0%", transition: { duration: 0.4, ease: "easeOut", when: "beforeChildren", staggerChildren: 0.06 } },
    ...
    const ctaMobileVariant = {
        ...
        visible: { y: 0, opacity: 1, transition: { delay: navLinks.length * 0.06 + 0.1, duration: 0.4, ease: "easeOut" } },
    };
    ...
    return (
        <>
            <header
                className={`fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl shadow-sm transition-transform duration-300 ease-out w-full ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}
                dir="rtl"
            >
                                ...
                                <li key={link.name}>
                                    <Link
                                        ...
                                        className={`group relative text-base font-semibold transition-colors duration-300 flex items-center whitespace-nowrap
                                            ${isActiveLink(link.href) ? 'text-brand-cyan' : 'text-brand-slate hover:text-brand-cyan'}`}
                                        onClick={(e) => {
                                    ...
                                    >
                                        {link.icon && (<Icon name={link.icon as any} className="inline-block w-4 h-4 ml-1 -mt-1" />)}
                                        {link.name}
                                        <span className={`absolute bottom-0 left-0 w-full h-[2px] bg-brand-cyan origin-left transition-transform duration-200 ease-out
                                            ${isActiveLink(link.href) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></span>
                                    </Link>
                                </li>
                            ))}
                        </ul>
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 bg-brand-cream z-[60] flex flex-col pt-4 pb-12 px-6 overflow-y-auto min-h-screen h-full"
                        dir="rtl"
                    >
                        ...
                        <nav className="flex flex-col items-end gap-y-2 w-full">
                            {navLinks.map((link) => (
                                <motion.div key={link.name} variants={mobileLinkVariants} className="w-full text-right">
                                    ...
                                    >
                                        {link.icon && (<Icon name={link.icon as any} className="inline-block w-5 h-5 mr-2 -mt-1" />)}
                                        {link.name}
                                    </Link>
                                </motion.div>
                            ))}
