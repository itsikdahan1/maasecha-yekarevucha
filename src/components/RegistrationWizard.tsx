    ...
    return (
        <div className="bg-white p-8 sm:p-10 rounded-2xl shadow-2xl border border-slate-200/80" dir="rtl">
            <Step currentStep={step} stepNumber={1} title="יצירת חשבון">
                <p className="text-center text-brand-dark/70 mb-6">נתחיל עם הפרטים הבסיסיים. הפרטים שלך בטוחים איתנו.</p>
                ...
                <button onClick={nextStep} className="w-full mt-8 bg-brand-dark text-white py-3 rounded-lg font-semibold">המשך לשלב הבא</button>
            </Step>

            <Step currentStep={step} stepNumber={2} title="ספר/י לנו עליך">
                <p className="text-center text-brand-dark/70 mb-6">בלי לחץ, פשוט כתוב/י בכמה מילים מה חשוב לך, מה את/ה אוהב/ת, ומה את/ה מחפש/ת.</p>
                ...
                <div className="flex justify-between mt-8">
                    ...
                    <button onClick={nextStep} className="bg-brand-dark text-white py-3 px-6 rounded-lg font-semibold">המשך לשלב הבא</button>
                </div>
            </Step>

            <Step currentStep={step} stepNumber={3} title="הקסם של היועץ החכם">
                <p className="text-center text-brand-dark/70 mb-6">זהו הערך המוסף שלנו. לחץ/י על הכפתור וראה/י איך היועץ החכם שלנו הופך את הטקסט שלך לפרופיל מרגש.</p>
                <button onClick={handleGenerateAiSuggestion} disabled={isLoading} className="w-full bg-brand-cyan text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2 disabled:bg-cyan-300">
                ...
                {aiSuggestion && (
                    <div className="mt-6 p-4 bg-brand-cream/60 rounded-lg border border-slate-200">
                        ...
                        <p dangerouslySetInnerHTML={renderFormattedText(aiSuggestion)} className="mt-2 text-brand-dark/80"></p>
                    </div>
                )}
                <div className="flex justify-between mt-8">
                    <button onClick={prevStep} className="text-slate-600 font-semibold">חזרה</button>
                    <button onClick={nextStep} className="bg-brand-dark text-white py-3 px-6 rounded-lg font-semibold">המשך לשלב הסופי</button>
                </div>
            </Step>
            ...
            <Step currentStep={step} stepNumber={4} title="הפרופיל שלך כמעט מוכן!">
                <div className="text-center">
                    ...
                    <Link href="/how-it-works#events" className="inline-block mt-8 bg-brand-cyan text-white py-4 px-8 rounded-lg font-bold text-lg shadow-lg hover:bg-cyan-600 transition-colors">
