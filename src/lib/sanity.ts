...
export async function getPosts(): Promise<Post[]> { return client.fetch<Post[]>(`*[_type == "post"] | order(publishedAt desc) { _id, title, slug, "authorName": author->name, "categoryName": category->name, publishedAt, excerpt, mainImage }`); }

// @ts-ignore: This is a temporary workaround for a TypeScript issue with Next.js 15 beta and Pick utility type.
